...
namespace system
{
    class Utils
    {
        ...
        static public void ErrorMsg(string _log)
        {
        ...
        static public double Max(double lsb, double rsb)
        {
            return (lsb > rsb) ? lsb : rsb;
        }
        static public double Min(double lsb, double rsb)
        {
            return (lsb < rsb) ? lsb : rsb;
        }
        ...
        static public byte[] Serialize(object _obj)
        {
            var buffer = new byte[Marshal.SizeOf(_obj)];
            var gch = GCHandle.Alloc(buffer, GCHandleType.Pinned);
            var pBuffer = gch.AddrOfPinnedObject();
            Marshal.StructureToPtr(_obj, pBuffer, false);
            gch.Free();

            return buffer;
        }
        static public void Deserialize<T>(T _obj, byte[] data)
        {
